export const hours = [
  {
    name: "子",
    range: "23:00—01:00",
    element: "木",
    tone: "角",
    meridian: "足少阳胆经",
    mood: "深夜，山色入墨",
  },
  {
    name: "丑",
    range: "01:00—03:00",
    element: "木",
    tone: "角",
    meridian: "足厥阴肝经",
    mood: "夜深，万籁未明",
  },
  {
    name: "寅",
    range: "03:00—05:00",
    element: "金",
    tone: "商",
    meridian: "手太阴肺经",
    mood: "天将亮，气息初醒",
  },
  {
    name: "卯",
    range: "05:00—07:00",
    element: "金",
    tone: "商",
    meridian: "手阳明大肠经",
    mood: "晨光，露水未干",
  },
  {
    name: "辰",
    range: "07:00—09:00",
    element: "土",
    tone: "宫",
    meridian: "足阳明胃经",
    mood: "晨起，山风渐暖",
  },
  {
    name: "巳",
    range: "09:00—11:00",
    element: "土",
    tone: "宫",
    meridian: "足太阴脾经",
    mood: "日上，溪石有光",
  },
  {
    name: "午",
    range: "11:00—13:00",
    element: "火",
    tone: "徵",
    meridian: "手少阴心经",
    mood: "正午，光线丰沛",
  },
  {
    name: "未",
    range: "13:00—15:00",
    element: "火",
    tone: "徵",
    meridian: "手太阳小肠经",
    mood: "午后，风过长坡",
  },
  {
    name: "申",
    range: "15:00—17:00",
    element: "水",
    tone: "羽",
    meridian: "足太阳膀胱经",
    mood: "日斜，云影渐长",
  },
  {
    name: "酉",
    range: "17:00—19:00",
    element: "水",
    tone: "羽",
    meridian: "足少阴肾经",
    mood: "暮色，远钟将起",
  },
  {
    name: "戌",
    range: "19:00—21:00",
    element: "火",
    tone: "徵",
    meridian: "手厥阴心包经",
    mood: "入夜，灯火初上",
  },
  {
    name: "亥",
    range: "21:00—23:00",
    element: "火",
    tone: "徵",
    meridian: "手少阳三焦经",
    mood: "夜静，月行中天",
  },
];

export const tracks = [
  {
    id: 1,
    title: "松影初生",
    sub: "古琴 / 箫 / 埙",
    tone: "角",
    hour: "子时",
    duration: "02:53",
    length: 173,
    audioSrc: "/audio/01-stone-and-pine-at-midnight.mp3",
    coverSrc: "/art/covers/01.png",
    detail: "深夜山色入墨，琴声从松影之间刚刚生出。",
  },
  {
    id: 2,
    title: "埙息未明",
    sub: "埙 / 古琴 / 箫",
    tone: "角",
    hour: "丑时",
    duration: "02:53",
    length: 173,
    audioSrc: "/audio/02-earth-breathing-before-dawn.mp3",
    coverSrc: "/art/covers/02.png",
    detail: "黎明尚未抵达，埙声托住土色与缓慢呼吸。",
  },
  {
    id: 3,
    title: "窗隙见白",
    sub: "箫 / 笛 / 古琴",
    tone: "商",
    hour: "寅时",
    duration: "02:51",
    length: 171,
    audioSrc: "/audio/03-pale-ink-awakening.mp3",
    coverSrc: "/art/covers/03.png",
    detail: "墨蓝向青灰打开，第一束光从窗隙里显现。",
  },
  {
    id: 4,
    title: "露落清商",
    sub: "笛 / 箫 / 磬",
    tone: "商",
    hour: "卯时",
    duration: "02:55",
    length: 175,
    audioSrc: "/audio/04-morning-light-on-still-water.mp3",
    coverSrc: "/art/covers/04.png",
    detail: "露水从草叶落下，晨风清澈而有边界。",
  },
  {
    id: 5,
    title: "山腹承光",
    sub: "笙 / 埙 / 古琴",
    tone: "宫",
    hour: "辰时",
    duration: "02:54",
    length: 174,
    audioSrc: "/audio/05-shan-fu-cheng-guang.mp3",
    coverSrc: "/art/covers/05.png",
    detail: "山腹承住日光，声音回到一个温和的中心。",
  },
  {
    id: 6,
    title: "溪石温声",
    sub: "古琴 / 笙 / 箫",
    tone: "宫",
    hour: "巳时",
    duration: "02:52",
    length: 172,
    audioSrc: "/audio/06-warm-stones-in-the-current.mp3",
    coverSrc: "/art/covers/06.png",
    detail: "溪边石面渐暖，水声把稳定的气息带向远处。",
  },
  {
    id: 7,
    title: "日中有弦",
    sub: "琵琶 / 笛 / 古琴",
    tone: "徵",
    hour: "午时",
    duration: "02:55",
    length: 175,
    audioSrc: "/audio/07-sunlight-across-the-ridge.mp3",
    coverSrc: "/art/covers/07.png",
    detail: "正午光线丰沛，弦上的颗粒清晰而不喧闹。",
  },
  {
    id: 8,
    title: "坡风流金",
    sub: "琵琶 / 笛 / 古琴",
    tone: "徵",
    hour: "未时",
    duration: "02:55",
    length: 175,
    audioSrc: "/audio/08-jin-feng-fu-guo-shan-ji.mp3",
    coverSrc: "/art/covers/08.png",
    detail: "午后风过长坡，声音带着向前流动的金色。",
  },
  {
    id: 9,
    title: "云过深碧",
    sub: "箫 / 古琴 / 埙",
    tone: "羽",
    hour: "申时",
    duration: "02:53",
    length: 173,
    audioSrc: "/audio/09-lv-shui-wan-qing.mp3",
    coverSrc: "/art/covers/09.png",
    detail: "云影变长，光在深碧的水面上缓慢移动。",
  },
  {
    id: 10,
    title: "钟沉暮水",
    sub: "箫 / 磬 / 古琴",
    tone: "羽",
    hour: "酉时",
    duration: "02:54",
    length: 174,
    audioSrc: "/audio/10-where-the-bell-descends.mp3",
    coverSrc: "/art/covers/10.png",
    detail: "一声远钟沉入暮水，山形在薄雾中逐渐收拢。",
  },
  {
    id: 11,
    title: "灯影未冷",
    sub: "古琴 / 箫 / 琵琶",
    tone: "徵",
    hour: "戌时",
    duration: "02:55",
    length: 175,
    audioSrc: "/audio/11-beneath-the-flickering-wick.mp3",
    coverSrc: "/art/covers/11.png",
    detail: "灯影与夜色相接，火的温度仍在，却已向内收拢。",
  },
  {
    id: 12,
    title: "月行归寂",
    sub: "箫 / 古琴 / 埙",
    tone: "徵",
    hour: "亥时",
    duration: "02:53",
    length: 173,
    audioSrc: "/audio/12-moonlight-on-the-high-ridge.mp3",
    coverSrc: "/art/covers/12.png",
    detail: "月光穿过窗棂，最后的暖意回到一处安静中心。",
  },
  {
    id: 13,
    title: "暗松入夜",
    sub: "古琴 / 箫 / 埙",
    tone: "角",
    hour: "子时 · 备用",
    duration: "02:45",
    length: 165,
    audioSrc: "/audio/13-under-the-dark-pine.mp3",
    coverSrc: "/art/covers/13.png",
    detail: "正式曲目的备用变体，松影更深，角音在夜色中向内生发。",
  },
  {
    id: 14,
    title: "晨钟未鸣",
    sub: "埙 / 古琴 / 箫",
    tone: "角",
    hour: "丑时 · 备用",
    duration: "02:52",
    length: 172,
    audioSrc: "/audio/14-before-the-morning-bell.mp3",
    coverSrc: "/art/covers/14.png",
    detail: "黎明尚未抵达，埙息把木的萌动藏在将明未明之间。",
  },
  {
    id: 15,
    title: "竹间有雾",
    sub: "箫 / 笛 / 古琴",
    tone: "商",
    hour: "寅时 · 备用",
    duration: "02:28",
    length: 148,
    audioSrc: "/audio/15-between-bamboo-and-fog.mp3",
    coverSrc: "/art/covers/15.png",
    detail: "雾气穿过竹间，清晨的边界在商音中慢慢显形。",
  },
  {
    id: 16,
    title: "山上露痕",
    sub: "笛 / 箫 / 磬",
    tone: "商",
    hour: "卯时 · 备用",
    duration: "02:44",
    length: 164,
    audioSrc: "/audio/16-dew-upon-the-mountain.mp3",
    coverSrc: "/art/covers/16.png",
    detail: "露水沿山石下行，笛声清亮而克制，留下细微的金石回响。",
  },
  {
    id: 17,
    title: "谷中晨光",
    sub: "笙 / 埙 / 古琴",
    tone: "宫",
    hour: "辰时 · 备用",
    duration: "02:49",
    length: 169,
    audioSrc: "/audio/17-morning-light-in-the-valley.mp3",
    coverSrc: "/art/covers/17.png",
    detail: "晨光落入谷地，声音回到一个温厚、安稳的中心。",
  },
  {
    id: 18,
    title: "水止之处",
    sub: "古琴 / 笙 / 箫",
    tone: "宫",
    hour: "巳时 · 备用",
    duration: "02:49",
    length: 169,
    audioSrc: "/audio/18-where-the-water-rests.mp3",
    coverSrc: "/art/covers/18.png",
    detail: "水流抵达平缓处，宫音托住清晰而不急的呼吸。",
  },
  {
    id: 19,
    title: "山巅正中",
    sub: "琵琶 / 笛 / 古琴",
    tone: "徵",
    hour: "午时 · 备用",
    duration: "02:49",
    length: 169,
    audioSrc: "/audio/19-zenith-of-the-summit.mp3",
    coverSrc: "/art/covers/19.png",
    detail: "日光抵达山巅，徵音明亮而不喧，弦上的颗粒向远处展开。",
  },
  {
    id: 20,
    title: "坡上金风",
    sub: "琵琶 / 笛 / 古琴",
    tone: "徵",
    hour: "未时 · 备用",
    duration: "02:49",
    length: 169,
    audioSrc: "/audio/20-golden-wind-on-the-hillside.mp3",
    coverSrc: "/art/covers/20.png",
    detail: "午后风过长坡，明亮的徵音带着轻快而稳的流动。",
  },
  {
    id: 21,
    title: "山水泊然",
    sub: "箫 / 古琴 / 埙",
    tone: "羽",
    hour: "申时 · 备用",
    duration: "02:45",
    length: 165,
    audioSrc: "/audio/21-mountain-waters-at-rest.mp3",
    coverSrc: "/art/covers/21.png",
    detail: "山水暂泊，深碧的余韵在羽音里缓慢下降。",
  },
  {
    id: 22,
    title: "暮山水色",
    sub: "箫 / 磬 / 古琴",
    tone: "羽",
    hour: "酉时 · 备用",
    duration: "02:31",
    length: 151,
    audioSrc: "/audio/22-dusk-on-the-mountain-water.mp3",
    coverSrc: "/art/covers/22.png",
    detail: "暮色映入山水，箫声把一天的明亮收束成远处的水色。",
  },
  {
    id: 23,
    title: "迟月过岭",
    sub: "古琴 / 箫 / 琵琶",
    tone: "徵",
    hour: "戌时 · 备用",
    duration: "02:45",
    length: 165,
    audioSrc: "/audio/23-late-moon-on-the-ridge.mp3",
    coverSrc: "/art/covers/23.png",
    detail: "月亮迟迟越过山岭，火的余温在夜色中向内保存。",
  },
  {
    id: 24,
    title: "绢上月光",
    sub: "箫 / 古琴 / 埙",
    tone: "徵",
    hour: "亥时 · 备用",
    duration: "02:37",
    length: 157,
    audioSrc: "/audio/24-moonlight-on-the-silk.mp3",
    coverSrc: "/art/covers/24.png",
    detail: "月光落在素绢上，最后一缕温暖回到安静的中心。",
  },
  {
    id: 25,
    title: "疏肝理气意象",
    sub: "古琴 / 箫 / 埙",
    tone: "角",
    hour: "治法意象",
    duration: "02:59",
    length: 179,
    audioSrc: "/audio/25-branches-toward-light.mp3",
    coverSrc: "/art/covers/25.png",
    detail: "枝条向光展开，以角音和春木意象表现舒展、开阔与生发。",
  },
  {
    id: 26,
    title: "宁心安神意象",
    sub: "古琴 / 琵琶 / 箫",
    tone: "徵",
    hour: "治法意象",
    duration: "02:53",
    length: 173,
    audioSrc: "/audio/26-light-of-a-paper-lantern.mp3",
    coverSrc: "/art/covers/26.png",
    detail: "纸灯笼的温光逐渐安定，以徵音表现柔和、稳定与收声。",
  },
  {
    id: 27,
    title: "健脾和中意象",
    sub: "古琴 / 埙 / 笙",
    tone: "宫",
    hour: "治法意象",
    duration: "02:55",
    length: 175,
    audioSrc: "/audio/27-clear-water-surface.mp3",
    coverSrc: "/art/covers/27.png",
    detail: "清水平面回到中央，以宫音表现承载、平衡与温和的稳定感。",
  },
  {
    id: 28,
    title: "宣肺理气意象",
    sub: "箫 / 古琴 / 磬",
    tone: "商",
    hour: "治法意象",
    duration: "02:51",
    length: 171,
    audioSrc: "/audio/28-mountain-rain-pavilion.mp3",
    coverSrc: "/art/covers/28.png",
    detail: "山雨穿过亭台，以商音表现清透、舒展与缓慢下行。",
  },
  {
    id: 29,
    title: "滋阴潜阳意象",
    sub: "埙 / 古琴 / 箫",
    tone: "羽",
    hour: "治法意象",
    duration: "02:56",
    length: 176,
    audioSrc: "/audio/29-night-river-returning-home.mp3",
    coverSrc: "/art/covers/29.png",
    detail: "夜河向归处缓慢流动，以羽音表现深远、内收与静水归根。",
  },
];

export const tones = [
  {
    name: "角",
    pinyin: "jué",
    element: "木",
    modernNote: "E4",
    pitchPool: "C4 D4 E4 G4 A4",
    colorName: "苔青",
    color: "#668f78",
    note: "清远 · 生发",
    theme: "wood",
  },
  {
    name: "徵",
    pinyin: "zhǐ",
    element: "火",
    modernNote: "G4",
    pitchPool: "C4 D4 E4 G4 A4",
    colorName: "微朱",
    color: "#b96952",
    note: "明亮 · 流动",
    theme: "fire",
  },
  {
    name: "宫",
    pinyin: "gōng",
    element: "土",
    modernNote: "C4",
    pitchPool: "C4 D4 E4 G4 A4",
    colorName: "沙金",
    color: "#ad8b55",
    note: "厚重 · 平和",
    theme: "earth",
  },
  {
    name: "商",
    pinyin: "shāng",
    element: "金",
    modernNote: "D4",
    pitchPool: "C4 D4 E4 G4 A4",
    colorName: "月白",
    color: "#869590",
    note: "清肃 · 空灵",
    theme: "metal",
  },
  {
    name: "羽",
    pinyin: "yǔ",
    element: "水",
    modernNote: "A4",
    pitchPool: "C4 D4 E4 G4 A4",
    colorName: "墨蓝",
    color: "#4d7180",
    note: "深远 · 含蓄",
    theme: "water",
  },
];

export const sourceTexts = [
  {
    id: "wuxing",
    index: "01",
    concept: "五行",
    title: "五行各有其性",
    source: "《尚书·洪范》",
    sourceUrl: "https://ctext.org/shang-shu/great-plan/zh",
    label: "水 · 火 · 木 · 金 · 土",
    quote:
      "五行：一曰水，二曰火，三曰木，四曰金，五曰土。水曰润下，火曰炎上，木曰曲直，金曰从革，土爰稼穑。",
    note: "原文先列五行之名，继而说它们的性状。听时以此作为五行与声音关系的观念入口。",
  },
  {
    id: "season-tones",
    index: "02",
    concept: "时序与五音",
    title: "月令各有其音",
    source: "《礼记·月令》",
    sourceUrl: "https://ctext.org/liji/yue-ling/zh",
    label: "春角 · 夏徵 · 中宫 · 秋商 · 冬羽",
    quoteLines: [
      "孟春之月……其音角。",
      "孟夏之月……其音徵。",
      "中央土……其音宫。",
      "孟秋之月……其音商。",
      "孟冬之月……其音羽。",
    ],
    note: "此处为与五音相关的原文节录，省略号表示删节；原篇还将月令与日、律、味等并列。",
  },
  {
    id: "five-sounds",
    index: "03",
    concept: "五音",
    title: "五声各有所主",
    source: "《礼记·乐记》",
    sourceUrl: "https://ctext.org/liji/yue-ji/zh",
    label: "宫 · 商 · 角 · 徵 · 羽",
    quote: "宫为君，商为臣，角为民，徵为事，羽为物。五者不乱，则无怗懘之音矣。",
    note: "《乐记》把五声放进君、臣、民、事、物的秩序中，强调的是礼乐观念中的对应关系。",
  },
  {
    id: "music-heart",
    index: "04",
    concept: "声音与心",
    title: "音乐从感受中发生",
    source: "《礼记·乐记》",
    sourceUrl: "https://ctext.org/liji/yue-ji/zh",
    label: "音 · 心 · 物",
    quote: "乐者，音之所由生也；其本在人心之感于物也。",
    note: "这句话让声音回到人的感受与外物之间，也是听时把自然意象写入曲目的文字依据。",
  },
  {
    id: "five-organs",
    index: "05",
    concept: "五脏与五音",
    title: "音、声与脏腑相应",
    source: "《黄帝内经·素问·阴阳应象大论》",
    sourceUrl: "https://ctext.org/huangdi-neijing/yin-yang-ying-xiang-da-lun/zhs",
    label: "角呼 · 徵笑 · 宫歌 · 商哭 · 羽呻",
    quote:
      "肝在音为角，在声为呼；心在音为徵，在声为笑；脾在音为宫，在声为歌；肺在音为商，在声为哭；肾在音为羽，在声为呻。",
    note: "这里保存古籍中的音声对应，不将古代理论当作现代医学结论，也不替代任何健康判断。",
  },
  {
    id: "meridians",
    index: "06",
    concept: "经脉",
    title: "经脉贵在通行",
    source: "《黄帝内经·灵枢·经脉》",
    sourceUrl: "https://ctext.org/huangdi-neijing/jing-mai/zh",
    label: "行血气 · 营阴阳",
    quote: "经脉者，所以能决死生、处百病、调虚实，不可不通。",
    note: "原文讨论经脉的作用。听时只借用“通行”的文化意象来组织时间与声音，不作医疗解释。",
  },
  {
    id: "five-sounds-and-virtues",
    index: "07",
    concept: "乐与人",
    title: "五音动而正其心",
    source: "《史记·乐书》",
    sourceUrl: "https://ctext.org/shiji/yue-shu/zh",
    label: "宫圣 · 商义 · 角仁 · 徵礼 · 羽智",
    quote:
      "故宫动脾而和正圣，商动肺而和正义，角动肝而和正仁，徵动心而和正礼，羽动肾而和正智。",
    note: "《乐书》将五音、五脏与五常并列，是听时理解“声音不只是标签”的另一条文献线索。",
  },
  {
    id: "branches-and-pitches",
    index: "08",
    concept: "十二辰",
    title: "地支与律吕相参",
    source: "《通典·乐三》",
    sourceUrl: "https://ctext.org/tongdian/143/zhs",
    label: "子黄钟 · 丑大吕",
    quote:
      "先王通于伦理，以候气之管为乐声之均，吹建子之律，以子为黄钟，十一月之辰名子。丑为大吕，十二月之辰名丑。",
    note: "这是后世乐律论述中的相关记载；其中的月辰与律吕，不直接等同于今天的标准时间刻度。",
  },
  {
    id: "natural-signs",
    index: "09",
    concept: "自然意象",
    title: "春气从万物中显现",
    source: "《礼记·月令》",
    sourceUrl: "https://ctext.org/liji/yue-ling/zh",
    label: "风 · 虫 · 鱼 · 雁",
    quote: "东风解冻，蛰虫始振，鱼上冰，獭祭鱼，鸿雁来。",
    note: "自然物候在原文中不是装饰性的风景，而是时序变化的可见迹象；首页的自然意象由此延展开来。",
  },
];

export const designStates = [
  {
    id: "growth",
    index: "01",
    name: "生发",
    subtitle: "向外生长",
    range: "05:00 — 09:00",
    trigger: "首次进入",
    description: "从一个声音的种子开始，逐步展开更多细节。",
    hint: "渐进显现 · 向上扩张",
    accent: "#698976",
  },
  {
    id: "flow",
    index: "02",
    name: "流行",
    subtitle: "顺势而行",
    range: "09:00 — 13:00",
    trigger: "连续聆听",
    description: "让曲目、时辰与导航沿同一条声流自然经过。",
    hint: "连续流动 · 手势导航",
    accent: "#667d84",
  },
  {
    id: "transform",
    index: "03",
    name: "化育",
    subtitle: "关系生成",
    range: "13:00 — 17:00",
    trigger: "切换五音",
    description: "把五音、时辰、情绪与场景重新组合成一次聆听。",
    hint: "卡片融合 · 关系重组",
    accent: "#9e8261",
  },
  {
    id: "converge",
    index: "04",
    name: "收敛",
    subtitle: "回到一处",
    range: "17:00 — 21:00",
    trigger: "进入专注",
    description: "收起多余信息，只留下声音、脉搏与正在发生的此刻。",
    hint: "界面减法 · 专注聆听",
    accent: "#c07a5d",
  },
  {
    id: "archive",
    index: "05",
    name: "收藏",
    subtitle: "向内保存",
    range: "21:00 — 05:00",
    trigger: "播放完成",
    description: "进入个人声音档案，保存那些值得再次相遇的时刻。",
    hint: "记忆层叠 · 安静持久",
    accent: "#8994a1",
  },
];

export const formatClock = (date) =>
  date.toLocaleTimeString("zh-CN", {
    hour: "2-digit",
    minute: "2-digit",
    hour12: false,
  });

const heavenlyStems = "甲乙丙丁戊己庚辛壬癸";
const earthlyBranches = "子丑寅卯辰巳午未申酉戌亥";

const lunarDayText = (value) => {
  const day = Number(value);
  if (!day) return value;
  if (day <= 10) return `初${"一二三四五六七八九十"[day - 1]}`;
  if (day < 20) return `十${"一二三四五六七八九"[day - 11]}`;
  if (day === 20) return "二十";
  if (day < 30) return `廿${"一二三四五六七八九"[day - 21]}`;
  return "三十";
};

const fallbackGanzhiYear = (year) =>
  `${heavenlyStems[((year - 4) % 10 + 10) % 10]}${earthlyBranches[((year - 4) % 12 + 12) % 12]}年`;

const ganzhiAt = (index) =>
  `${heavenlyStems[index % 10]}${earthlyBranches[index % 12]}`;

const getGanzhiPillars = (date) => {
  const year = date.getFullYear();
  const solarYear = date < new Date(year, 1, 4) ? year - 1 : year;
  const yearIndex = ((solarYear - 4) % 60 + 60) % 60;
  const yearStem = yearIndex % 10;
  const yinMonthStems = [2, 4, 6, 8, 0, 2, 4, 6, 8, 0];
  const monthBoundaries = [
    [2, 4, 2],
    [3, 6, 3],
    [4, 5, 4],
    [5, 5, 5],
    [6, 5, 6],
    [7, 7, 7],
    [8, 7, 8],
    [9, 7, 9],
    [10, 8, 10],
    [11, 7, 11],
    [12, 7, 0],
    [1, 5, 1],
  ];
  let currentMonth = { date: new Date(year - 1, 11, 7), branch: 0 };
  monthBoundaries.forEach(([month, day, branch]) => {
    const boundary = new Date(year, month - 1, day);
    if (boundary <= date && boundary > currentMonth.date) {
      currentMonth = { date: boundary, branch };
    }
  });
  const monthOffset = (currentMonth.branch - 2 + 12) % 12;
  const monthStem = (yinMonthStems[yearStem] + monthOffset) % 10;

  const pillarDate = new Date(date);
  if (pillarDate.getHours() >= 23) pillarDate.setDate(pillarDate.getDate() + 1);
  let y = pillarDate.getFullYear();
  let m = pillarDate.getMonth() + 1;
  const d = pillarDate.getDate();
  if (m <= 2) {
    y -= 1;
    m += 12;
  }
  const century = Math.floor(y / 100);
  const correction = 2 - century + Math.floor(century / 4);
  const julianDay =
    Math.floor(365.25 * (y + 4716)) +
    Math.floor(30.6001 * (m + 1)) +
    d +
    correction -
    1524.5;
  const dayIndex = (Math.floor(julianDay + 0.5) + 49) % 60;
  const dayStem = dayIndex % 10;
  const ziHourStems = [0, 2, 4, 6, 8, 0, 2, 4, 6, 8];
  const hourBranch = Math.floor(((date.getHours() + 1) % 24) / 2);
  const hourStem = (ziHourStems[dayStem] + hourBranch) % 10;

  return {
    year: `${ganzhiAt(yearIndex)}年`,
    month: `${heavenlyStems[monthStem]}${earthlyBranches[currentMonth.branch]}月`,
    day: `${ganzhiAt(dayIndex)}日`,
    hour: `${heavenlyStems[hourStem]}${earthlyBranches[hourBranch]}时`,
  };
};

export function getCalendarMeta(date) {
  const gregorian = `${date.getFullYear()}年${String(date.getMonth() + 1).padStart(
    2,
    "0",
  )}月${String(date.getDate()).padStart(2, "0")}日`;
  const weekday = new Intl.DateTimeFormat("zh-CN", {
    weekday: "long",
  }).format(date);
  const pillars = getGanzhiPillars(date);
  let lunar = "农历日期";
  let ganzhiYear = pillars.year || fallbackGanzhiYear(date.getFullYear());

  try {
    const parts = new Intl.DateTimeFormat("zh-CN-u-ca-chinese", {
      year: "numeric",
      month: "long",
      day: "numeric",
    }).formatToParts(date);
    const month = parts.find((part) => part.type === "month")?.value;
    const day = parts.find((part) => part.type === "day")?.value;
    const yearName = parts.find((part) => part.type === "yearName")?.value;
    if (month && day) lunar = `农历${month}${lunarDayText(day)}`;
    if (yearName && !pillars.year) ganzhiYear = `${yearName}年`;
  } catch {
    // Older browsers can fall back to the stable Gregorian and hour labels.
  }

  return {
    gregorian,
    weekday,
    lunar,
    ganzhiYear,
    hourPillar: pillars.hour,
    ganzhi: `${pillars.year} · ${pillars.month} · ${pillars.day} · ${pillars.hour}`,
  };
}

export function getCurrentHour() {
  const now = new Date();
  const hour = now.getHours();
  const index = hour === 23 ? 0 : Math.floor((hour + 1) / 2);
  return { data: hours[index], index, now };
}

export function getTimeState(date) {
  const hour = date.getHours();
  if (hour >= 5 && hour < 9) return designStates[0];
  if (hour >= 9 && hour < 13) return designStates[1];
  if (hour >= 13 && hour < 17) return designStates[2];
  if (hour >= 17 && hour < 21) return designStates[3];
  return designStates[4];
}
